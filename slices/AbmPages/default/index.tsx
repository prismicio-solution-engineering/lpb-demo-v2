...
const AbmPages: FC<AbmPagesProps> = ({ slice }) => {
    ...
    const validPages = slice.primary.generated_page.filter((item) =>
                ...
                <div className="relative w-full z-10 p-0 sm:p-8 mt-8">
                    ...
                    >   
                        ...
                        <div key={index} className="snap-center shrink-0 w-[85vw] md:w-[420px] lg:w-[480px] transition-all duration-500 ease-out p-8 flex flex-col items-start gap-4 bg-[#1F1F1F] rounded-2xl border border-[#333333] hover:border-[#3BBB96] shadow-[0_10px_40px_rgba(0,0,0,0.6)] origin-bottom lg:origin-left">
                            ...
                            {/* {isFilled.keyText(item.company) && <h3 className="text-xl font-bold text-left text-[#FFFFFF]">{item.company}</h3>} */}
    
                            {isFilled.keyText(item.company) && (
                            <div className="w-full">
                                <span className="text-xs uppercase font-bold text-[#A4A4A4] mb-2 block text-left">Company</span>
                                <p className="text-[#FFFFFF] text-left text-sm">{item.company}</p>
                            </div>
                            )}

                            {isFilled.keyText(item.role) && (
                            <div className="w-full">
                                <span className="text-xs uppercase font-bold text-[#A4A4A4] mb-2 block text-left">Role</span>
                                <p className="text-[#FFFFFF] text-left text-sm">{item.role}</p>
                            </div>
                            )}
    
                            {isFilled.keyText(item.personalization_instructions) && (
                            <div className="w-full">
                                <span className="text-xs uppercase font-bold text-[#A4A4A4] mb-2 block text-left">Personalization Instructions</span>
                                <p className="text-[#FFFFFF] text-left text-sm">{item.personalization_instructions}</p>
                            </div>
                            )}
